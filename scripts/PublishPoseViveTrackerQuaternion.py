...
class PublishPoseViveTracker(Node):
    def __init__(self):
        ...
        self.offset_mat_map = {
            ...
            # "left_wrist": quaternion_offset(np.pi, 0.0, 0.0),
            # "right_wrist": quaternion_offset(np.pi, 0.0, 0.0),

            "left_wrist": quaternion_offset(0.0, 0.0, 0.0),
            "right_wrist": quaternion_offset(0.0, 0.0, 0.0),

            #"left_wrist": translation_matrix([0.0, 0.0, 0.05]),
            #"right_wrist": translation_matrix([0.0, 0.0, 0.05]),

            # "left_wrist": quaternion_offset(np.pi, 0.0, np.pi/2), # X = pi -> inverse l'axe 180deg ; Z = pi/2, rotation de 90deg pour changer daxe
            # "right_wrist": quaternion_offset(np.pi, 0.0, np.pi/2),

            # "left_wrist": quaternion_offset(0.0, 0.0, np.pi/2),
            # "right_wrist": quaternion_offset(0.0, 0.0, np.pi/2),
        }
    ...
    def process_single_device_data(self, device_idx, print_info=False):
        ...
        if body_part and is_pose_valid:
            ...
            print(f"{body_part} moving | pos={pos}") # test 2026-04-02
            print("RAW quat:", quaternion_from_matrix(self.root_mat @ pose_mat)) # test 2026-04-03
            print("FINAL quat:", quat) # test 2026-04-03
            ...
            self.get_logger().info(
                f"{body_part} quat = ({quat[0]:.3f}, {quat[1]:.3f}, {quat[2]:.3f}, {quat[3]:.3f})"
            )
