...
class PublishPoseViveTracker(Node):
    def __init__(self):
        ...
        #self.root_mat = euler_matrix(0.5 * np.pi, 0.0, 0.0) # og value
        self.root_mat = euler_matrix(0.5 * np.pi, 0.0, 0.5 * np.pi) # to test 13-03-2026
        ...
        self.offset_mat_map = {
            ...
            # "left_wrist": translation_matrix([0.0, 0.0, 0.05]), # og value
            # "right_wrist": quaternion_offset(np.pi, 0.0, 0.0), # og value

            "left_wrist": quaternion_offset(np.pi, 0.0, 0.0), # test selim 12-03-2026 
            "right_wrist": quaternion_offset(np.pi, 0.0, 0.0), # test selim 12-03-2026 
        }
        ...
        # read all VR devices, every frame 30Hz
        self.timer = self.create_timer(1.0 / 30.0, self._poll_devices)

        # identify which tracker is which body part
    ...
    # steam vr returns poses of all devices, this function is called @ self.timer
    ...
    def process_single_device_data(self, device_idx, print_info=False):
        ...
        pose_matrix_data = self.device_data_list[device_idx].mDeviceToAbsoluteTracking #steamVR pose matrix
        ...
        if body_part and is_pose_valid:
            ...
            pose_mat[3, 3] = 1.0
            pose_mat = self.root_mat @ pose_mat # global frame correction
            if body_part in self.offset_mat_map:
                pose_mat = pose_mat @ self.offset_mat_map[body_part] # tracker mounting correction : correct for how the tracker is physically mounted on the body
